...
describe("Tasks app", () => {
  ...
  it("Displays list of items", () => {
    cy.get("[data-testid=table-tasks]").should("be.visible")
  ...
  it("Search for specific task by title", () => {
    cy.get("[data-testid=search-input]").type("Review Codebase")
    cy.get("[data-testid=table-tasks] .ant-table-row-level-0 .ant-table-cell")
      .first()
  ...
  it("Search for specific task that not exists, by description", () => {
    ...
    cy.get("[data-testid=search-input]").type("Review Codebase")
    cy.wait(0)
    cy.get("[data-testid=table-tasks] .ant-table-row-level-0 .ant-table-cell").should("not.exist")
  })

  it("Add new task", () => {
    cy.contains("Create New Task").click()
    cy.get('[data-testid="input-title-task"]').type("New task title")
    cy.get('[data-testid="input-description-task"]').type("New task description")
  ...
  it("Edit existing task", () => {
    ...
    cy.get('[data-testid="input-title-task"]').as("inputTitle")
